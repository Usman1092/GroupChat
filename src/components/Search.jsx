...
export const Search = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const [userDocs, groupDocs] = await Promise.all([getDocs(usersQuery), getDocs(groupsQuery)]);

      const userResults = userDocs.docs.map(doc => ({ ...doc.data(), id: doc.id, type: 'user' }));
      const groupResults = groupDocs.docs.map(doc => ({ ...doc.data(), id: doc.id, type: 'group' }));
  ...
  const handleSelect = async (result) => {
    ...
    // if (groupData.exists()) {
    //   dispatch({ type: "CHANGE_GROUP", payload: { ...groupData.data(), id: groupId } });
    // }
              ...
              {result.type === "user" ? (
                <>
                  ...
                  <div className="userChatInfo">
                    <span className="span text-[#fff]">{result.displayName}</span>
                  </div>
                </>
              ) : (
